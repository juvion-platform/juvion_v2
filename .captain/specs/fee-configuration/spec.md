...
- **FeeAgreement override in invoice generation** — confirmed **not in scope for v1** after T10 discovered the override isn't actually wired into `generateSemesterInvoice` today (see OQ-15). The `FeeAgreement` model and admin CRUD exist, but the consumption path in invoice generation is a separate feature. Resolution: the spec's original claim "existing FeeAgreement override logic is unchanged" was inaccurate; there was nothing to preserve. Adding it back is a separate spec when the business needs it.
...
- **2026-04-21** — OQ-15 **resolved**: FeeAgreement override in invoice generation is confirmed out-of-scope for v1. Added to §NOT-For explicitly. The spec's original claim "existing FeeAgreement override logic is unchanged" was a misread — there was nothing to preserve because the override path doesn't exist in `generateSemesterInvoice` today. Re-introducing FeeAgreement consumption is a separate spec.

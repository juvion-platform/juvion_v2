...
// ─── Announcements ────────────────────────────────────────
export const listAnnouncements = (page = 1, limit = 20, category?: string, priority?: string, search?: string) =>
  api.get(`${BASE}/announcements`, { params: { page, limit, category, priority, ...(search ? { search } : {}) } }).then(r => r.data);
...
// ─── Circulars ────────────────────────────────────────────
export const listCirculars = (page = 1, limit = 20, targetAudience?: string, search?: string) =>
  api.get(`${BASE}/circulars`, { params: { page, limit, targetAudience, ...(search ? { search } : {}) } }).then(r => r.data);
...
// ─── Notifications ────────────────────────────────────────
export const listNotifications = (page = 1, limit = 20, status?: string, channel?: string, search?: string) =>
  api.get(`${BASE}/notifications`, { params: { page, limit, status, channel, ...(search ? { search } : {}) } }).then(r => r.data);
...
// ─── Feedback Surveys ─────────────────────────────────────
export const listFeedbackSurveys = (page = 1, limit = 20, status?: string, targetAudience?: string, search?: string) =>
  api.get(`${BASE}/feedback-surveys`, { params: { page, limit, status, targetAudience, ...(search ? { search } : {}) } }).then(r => r.data);
...
// ─── Survey Responses ─────────────────────────────────────
export const listSurveyResponses = (page = 1, limit = 20, surveyId?: string, search?: string) =>
  api.get(`${BASE}/survey-responses`, { params: { page, limit, surveyId, ...(search ? { search } : {}) } }).then(r => r.data);
...
// ─── Email Logs ───────────────────────────────────────────
export const listEmailLogs = (page = 1, limit = 20, status?: string, search?: string) =>
  api.get(`${BASE}/email-logs`, { params: { page, limit, status, ...(search ? { search } : {}) } }).then(r => r.data);
...
// ─── SMS Logs ─────────────────────────────────────────────
export const listSMSLogs = (page = 1, limit = 20, status?: string, search?: string) =>
  api.get(`${BASE}/sms-logs`, { params: { page, limit, status, ...(search ? { search } : {}) } }).then(r => r.data);
...
// ─── WhatsApp Logs ────────────────────────────────────────
export const listWhatsAppLogs = (page = 1, limit = 20, status?: string, search?: string) =>
  api.get(`${BASE}/whatsapp-logs`, { params: { page, limit, status, ...(search ? { search } : {}) } }).then(r => r.data);

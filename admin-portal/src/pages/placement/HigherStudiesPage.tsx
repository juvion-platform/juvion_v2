...
export default function HigherStudiesPage() {
  ...
  const [form, setForm] = useState({ studentId: '', examType: 'gate', examScore: '', targetUniversity: '', country: '', programmeApplied: '', status: 'preparing' });
  ...
  const createMut = useMutation({ mutationFn: createHigherStudiesApplication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['higher-studies'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateHigherStudiesApplication(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['higher-studies'] }); closeModal(); } });
  ...
  function openCreate() {
    setEditing(null);
    setForm({ studentId: '', examType: 'gate', examScore: '', targetUniversity: '', country: '', programmeApplied: '', status: 'preparing' });
    setModalOpen(true);
  }

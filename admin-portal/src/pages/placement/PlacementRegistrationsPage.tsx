...
const emptyForm = { jobPostingId: '', studentId: '', resumeUrl: '', status: 'registered' };
...
export default function PlacementRegistrationsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['placement-registrations', page, limit, search], queryFn: () => listPlacementRegistrations(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createPlacementRegistration, onSuccess: () => { qc.invalidateQueries({ queryKey: ['placement-registrations'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updatePlacementRegistration(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['placement-registrations'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search placement registrations…" className="w-56" />
      ...
      <DataTable
        ...
        emptyMessage={search ? `No placement registrations match “${search}”.` : 'No placement registrations yet.'}
      />
      <Pagination
        page={page}
        pages={data?.pages ?? 1}

...
export default function InternshipApplicationsPage() {
  ...
  const [form, setForm] = useState({ internshipId: '', studentId: '', status: 'applied' });
  ...
  const createMut = useMutation({ mutationFn: createInternshipApplication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['internship-applications'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateInternshipApplication(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['internship-applications'] }); closeModal(); } });
  ...
  function openCreate() {
    setEditing(null);
    setForm({ internshipId: '', studentId: '', status: 'applied' });
    setModalOpen(true);
  }
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Application' : 'New Application'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className={lbl}>Internship * <Link to="/placement/internships" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

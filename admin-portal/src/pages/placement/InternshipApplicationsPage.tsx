...
export default function InternshipApplicationsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['internship-applications', page, limit, search], queryFn: () => listInternshipApplications(page, limit, undefined, search) });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this application?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search internship applications…" className="w-56" />
      ...
      <DataTable
        ...
        emptyMessage={search ? `No internship applications match “${search}”.` : 'No internship applications yet.'}
      />
      <Pagination
        page={page}
        pages={data?.pages ?? 1}

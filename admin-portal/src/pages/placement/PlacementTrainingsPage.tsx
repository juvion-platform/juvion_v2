...
const emptyForm = { title: '', type: 'aptitude', trainer: '', startDate: '', endDate: '', status: 'planned' };
...
export default function PlacementTrainingsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['placement-trainings', page, limit, search], queryFn: () => listPlacementTrainings(page, limit, search) });
  ...
  const createMut = useMutation({ mutationFn: createPlacementTraining, onSuccess: () => { qc.invalidateQueries({ queryKey: ['placement-trainings'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updatePlacementTraining(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['placement-trainings'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search placement trainings…" className="w-56" />
        <button onClick={vem.openForCreate} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} className="text-white" /> New Training</button>
      </div>
      </div>
      <DataTable
        columns={columns}

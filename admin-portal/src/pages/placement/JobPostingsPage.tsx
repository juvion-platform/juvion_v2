...
const emptyForm = { placementSeasonId: '', companyId: '', role: '', description: '', packageLpa: '', registrationDeadline: '', maxPositions: '', status: 'draft' };
...
export default function JobPostingsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['job-postings', page, limit, search], queryFn: () => listJobPostings(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createJobPosting, onSuccess: () => { qc.invalidateQueries({ queryKey: ['job-postings'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateJobPosting(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['job-postings'] }); vem.close(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this job posting?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search job postings…" className="w-56" />
        <button onClick={vem.openForCreate} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} className="text-white" /> New Job Posting</button>
      </div>
      </div>
      <DataTable
        columns={columns}
        ...
        emptyMessage={search ? `No job postings match “${search}”.` : 'No job postings yet.'}
      />
      <Pagination
        page={page}
        pages={data?.pages ?? 1}
      ...
      />
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Job Posting')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className={lbl}>Season * {!vem.isView && <Link to="/placement/seasons" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link>}</label>

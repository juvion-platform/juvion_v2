...
const emptyForm = { companyId: '', title: '', description: '', stipend: '', durationWeeks: '', startDate: '', lastDateToApply: '', status: 'open' };
...
export default function InternshipPostingsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['internship-postings', page, limit, search], queryFn: () => listInternshipPostings(page, limit, search) });
  ...
  const createMut = useMutation({ mutationFn: createInternshipPosting, onSuccess: () => { qc.invalidateQueries({ queryKey: ['internship-postings'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateInternshipPosting(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['internship-postings'] }); vem.close(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this internship?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search internship postings…" className="w-56" />
        <button onClick={vem.openForCreate} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} className="text-white" /> New Internship</button>
      </div>
      </div>
      <DataTable
        columns={columns}
      ...
      />
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Internship')}>

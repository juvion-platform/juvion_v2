...
export default function StudentGrievancesPage() {
  ...
  const [form, setForm] = useState({ studentId: '', category: 'other', subject: '', description: '', priority: 'medium', assignedTo: '', status: 'open', resolution: '' });

  const { data, isLoading } = useQuery({ queryKey: ['student-grievances', page], queryFn: () => listStudentGrievances(page, 20) });
  ...
  const createMut = useMutation({ mutationFn: createStudentGrievance, onSuccess: () => { qc.invalidateQueries({ queryKey: ['student-grievances'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateStudentGrievance(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['student-grievances'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteStudentGrievance, onSuccess: () => { qc.invalidateQueries({ queryKey: ['student-grievances'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ studentId: '', category: 'other', subject: '', description: '', priority: 'medium', assignedTo: '', status: 'open', resolution: '' });
    setModalOpen(true);
  }

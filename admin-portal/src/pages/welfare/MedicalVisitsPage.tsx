...
export default function MedicalVisitsPage() {
  ...
  const [form, setForm] = useState({ personId: '', visitDate: '', complaint: '', diagnosis: '', prescription: '', referredTo: '', attendedBy: '', followUpDate: '' });

  const { data, isLoading } = useQuery({ queryKey: ['medical-visits', page], queryFn: () => listMedicalVisits(page, 20) });
  const { data: personsData } = useQuery({ queryKey: ['persons', 'all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const createMut = useMutation({ mutationFn: createMedicalVisit, onSuccess: () => { qc.invalidateQueries({ queryKey: ['medical-visits'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateMedicalVisit(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['medical-visits'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteMedicalVisit, onSuccess: () => { qc.invalidateQueries({ queryKey: ['medical-visits'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ personId: '', visitDate: '', complaint: '', diagnosis: '', prescription: '', referredTo: '', attendedBy: '', followUpDate: '' });
    setModalOpen(true);
  }
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this visit?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Medical Visit' : 'New Medical Visit'}>

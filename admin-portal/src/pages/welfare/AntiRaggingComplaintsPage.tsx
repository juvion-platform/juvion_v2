...
export default function AntiRaggingComplaintsPage() {
  ...
  const [form, setForm] = useState({ complainantId: '', isAnonymous: false, accusedIds: [] as string[], description: '', incidentDate: '', severity: 'minor', status: 'filed', committeeRemarks: '', actionTaken: '' });
  ...
  const createMut = useMutation({ mutationFn: createAntiRaggingComplaint, onSuccess: () => { qc.invalidateQueries({ queryKey: ['anti-ragging-complaints'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateAntiRaggingComplaint(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['anti-ragging-complaints'] }); closeModal(); } });
  ...
  function openCreate() {
    setEditing(null);
    setForm({ complainantId: '', isAnonymous: false, accusedIds: [], description: '', incidentDate: '', severity: 'minor', status: 'filed', committeeRemarks: '', actionTaken: '' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Complaint' : 'New Complaint'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className={lbl}>Accused Students <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

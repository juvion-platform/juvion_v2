...
const emptyForm = { studentId: '', parentId: '', facultyId: '', scheduledDate: '', agenda: '', notes: '', status: 'scheduled' };
...
export default function ParentMeetingsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['parent-meetings', page, limit, search], queryFn: () => listParentMeetings(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createParentMeeting, onSuccess: () => { qc.invalidateQueries({ queryKey: ['parent-meetings'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateParentMeeting(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['parent-meetings'] }); vem.close(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this meeting?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search parent meetings…" className="w-56" />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className={lbl}>Parent * {!vem.isView && <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link>}</label>
              ...
              <div><label className={lbl}>Faculty * {!vem.isView && <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link>}</label>

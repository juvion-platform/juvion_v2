...
export default function HealthRecordsPage() {
  ...
  const [form, setForm] = useState({ personId: '', bloodGroup: '', allergies: '', chronicConditions: '', emergencyContact: '', emergencyPhone: '', insuranceId: '' });

  const { data, isLoading } = useQuery({ queryKey: ['health-records', page], queryFn: () => listHealthRecords(page, 20) });
  const { data: personsData } = useQuery({ queryKey: ['persons', 'all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const createMut = useMutation({ mutationFn: createHealthRecord, onSuccess: () => { qc.invalidateQueries({ queryKey: ['health-records'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateHealthRecord(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['health-records'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteHealthRecord, onSuccess: () => { qc.invalidateQueries({ queryKey: ['health-records'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ personId: '', bloodGroup: '', allergies: '', chronicConditions: '', emergencyContact: '', emergencyPhone: '', insuranceId: '' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Health Record' : 'New Health Record'}>

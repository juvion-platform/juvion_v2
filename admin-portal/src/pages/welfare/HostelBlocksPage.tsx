...
export default function HostelBlocksPage() {
  ...
  const [form, setForm] = useState({ name: '', type: 'boys', totalRooms: '', wardenId: '', isActive: true });

  const { data, isLoading } = useQuery({ queryKey: ['hostel-blocks', page], queryFn: () => listHostelBlocks(page, 20) });
  const { data: personsData } = useQuery({ queryKey: ['persons', 'all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const createMut = useMutation({ mutationFn: createHostelBlock, onSuccess: () => { qc.invalidateQueries({ queryKey: ['hostel-blocks'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateHostelBlock(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['hostel-blocks'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteHostelBlock, onSuccess: () => { qc.invalidateQueries({ queryKey: ['hostel-blocks'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ name: '', type: 'boys', totalRooms: '', wardenId: '', isActive: true });
    setModalOpen(true);
  }
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this block?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Hostel Block' : 'New Hostel Block'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Warden <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

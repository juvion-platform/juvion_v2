...
const emptyForm = { blockId: '', roomNumber: '', floor: '', capacity: '', occupancy: '0', amenities: '', status: 'available' };
...
export default function HostelRoomsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['hostel-rooms', page, limit, search], queryFn: () => listHostelRooms(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createHostelRoom, onSuccess: () => { qc.invalidateQueries({ queryKey: ['hostel-rooms'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateHostelRoom(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['hostel-rooms'] }); vem.close(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this room?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search hostel rooms…" className="w-56" />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Hostel Room')}>

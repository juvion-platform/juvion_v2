...
export default function MeetingsPage() {
  ...
  const [form, setForm] = useState({ committeeId: '', meetingDate: '', agenda: '', minutes: '', decisions: '', nextMeetingDate: '', status: 'scheduled' });
  ...
  const createMut = useMutation({ mutationFn: createMeeting, onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-meetings'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateMeeting(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-meetings'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteMeeting, onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-meetings'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ committeeId: '', meetingDate: '', agenda: '', minutes: '', decisions: '', nextMeetingDate: '', status: 'scheduled' });
    setModalOpen(true);
  }
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Meeting' : 'New Meeting'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Committee * <Link to="/governance/committees" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

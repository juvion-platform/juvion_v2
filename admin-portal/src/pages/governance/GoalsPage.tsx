...
export default function GoalsPage() {
  ...
  const [form, setForm] = useState({ title: '', description: '', category: 'academic_excellence', targetDate: '', ownerId: '', status: 'active', kpis: '' });

  const { data, isLoading } = useQuery({ queryKey: ['gov-goals', page], queryFn: () => listGoals(page, 20) });
  const { data: persons } = useQuery({ queryKey: ['persons', 'all'], queryFn: () => listPersons(1, 200) });

  const createMut = useMutation({ mutationFn: createGoal, onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-goals'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateGoal(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-goals'] }); closeModal(); } });
  ...
  function openCreate() {
    setEditing(null);
    setForm({ title: '', description: '', category: 'academic_excellence', targetDate: '', ownerId: '', status: 'active', kpis: '' });
    setModalOpen(true);
  }
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this goal?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Goal' : 'New Goal'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={lbl}>Owner <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

...
const emptyForm = { title: '', category: 'academic', description: '', documentUrl: '', version: '1', effectiveDate: '', approvedBy: '', status: 'draft' };
...
export default function PoliciesPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['gov-policies', page, limit, search], queryFn: () => listPolicies(page, limit, undefined, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createPolicy, onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-policies'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updatePolicy(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-policies'] }); vem.close(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this policy?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search policies…" className="w-56" />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Policy')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className={lbl}>Approved By {!vem.isView && <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link>}</label>

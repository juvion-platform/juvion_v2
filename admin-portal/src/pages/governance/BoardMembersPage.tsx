...
export default function BoardMembersPage() {
  ...
  const [form, setForm] = useState({ personId: '', externalName: '', designation: '', role: 'member', appointedDate: '', tenure: '', isActive: true });

  const { data, isLoading } = useQuery({ queryKey: ['gov-board', page], queryFn: () => listBoardMembers(page, 20) });
  const { data: persons } = useQuery({ queryKey: ['persons', 'all'], queryFn: () => listPersons(1, 200) });

  const createMut = useMutation({ mutationFn: createBoardMember, onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-board'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateBoardMember(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-board'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteBoardMember, onSuccess: () => { qc.invalidateQueries({ queryKey: ['gov-board'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ personId: '', externalName: '', designation: '', role: 'member', appointedDate: '', tenure: '', isActive: true });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Board Member' : 'New Board Member'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Person (internal) <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

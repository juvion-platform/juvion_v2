...
const emptyForm = { buildingId:'', month:'', year:'', electricityUnits:'', electricityCost:'', waterUnits:'', waterCost:'', solarGenerated:'' };
...
export default function EnergyConsumptionPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['energy-consumptions', page, limit, search], queryFn: () => listEnergyConsumptions(page, limit, search) });
  const { data: bldgsData } = useQuery({ queryKey: ['buildings-ref','all'], queryFn: () => listBuildings(1, 200) });
  const bldgs = bldgsData?.items || [];

  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ buildingId: row.buildingId?._id||row.buildingId||'', month: String(row.month??''), year: String(row.year??''), electricityUnits: String(row.electricityUnits??''), electricityCost: String(row.electricityCost??''), waterUnits: String(row.waterUnits??''), waterCost: String(row.waterCost??''), solarGenerated: String(row.solarGenerated??'') }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createEnergyConsumption, onSuccess: () => { qc.invalidateQueries({ queryKey: ['energy-consumptions'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateEnergyConsumption(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['energy-consumptions'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search energy consumption…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No energy consumption match “${search}”.` : 'No energy consumption yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Energy Record')}>

...
export default function VisitorEntriesPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['visitor-entries', page, limit, search], queryFn: () => listVisitorEntries(page, limit, search) });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search visitor entries…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No visitor entries match “${search}”.` : 'No visitor entries yet.'}
      />

...
const emptyForm = { personId:'', personType:'student', type:'half_day', reason:'', outTime:'', expectedInTime:'', status:'requested' };
...
export default function GatePassesPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['gate-passes', page, limit, search], queryFn: () => listGatePasses(page, limit, search) });
  const { data: personsData } = useQuery({ queryKey: ['persons-ref','all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ personId: row.personId?._id||row.personId||'', personType: row.personType||'student', type: row.type||'half_day', reason: row.reason||'', outTime: row.outTime?.slice(0,16)||'', expectedInTime: row.expectedInTime?.slice(0,16)||'', status: row.status||'requested' }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createGatePass, onSuccess: () => { qc.invalidateQueries({ queryKey: ['gate-passes'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateGatePass(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['gate-passes'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search gate passes…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No gate passes match “${search}”.` : 'No gate passes yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Gate Pass')}>

...
const emptyForm = { eResourceId:'', personId:'', accessDate:'', duration:'' };
...
export default function EResourceAccessPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['e-resource-accesses', page, limit, search], queryFn: () => listEResourceAccesses(page, limit, search) });
  ...
  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ eResourceId: row.eResourceId?._id||row.eResourceId||'', personId: row.personId?._id||row.personId||'', accessDate: row.accessDate?.slice(0,10)||'', duration: String(row.duration??'') }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createEResourceAccess, onSuccess: () => { qc.invalidateQueries({ queryKey: ['e-resource-accesses'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateEResourceAccess(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['e-resource-accesses'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search e-resource access…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No e-resource access match “${search}”.` : 'No e-resource access yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('E-Resource Access')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className={lbl}>E-Resource * {!vem.isView && <Link to="/campus-ops/e-resources" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10}/></Link>}</label><select required value={form.eResourceId} onChange={e=>setForm(fm=>({...fm,eResourceId:e.target.value}))} className={inp}><option value="">Select...</option>{eResources.map((x:any)=><option key={x._id} value={x._id}>{x.title}</option>)}</select></div>

...
const STATUS_OPTS = ['issued','returned','overdue','lost'] as const;
...
export default function BookIssuesPage() {
  ...
  const [form, setForm] = useState({ bookId:'', issuedTo:'', issuedDate:'', dueDate:'', returnedDate:'', renewCount:'', fineAmount:'', status:'issued' });

  const { data, isLoading } = useQuery({ queryKey: ['book-issues', page], queryFn: () => listBookIssues(page, 20) });
  ...
  const createMut = useMutation({ mutationFn: createBookIssue, onSuccess: () => { qc.invalidateQueries({ queryKey: ['book-issues'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateBookIssue(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['book-issues'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteBookIssue, onSuccess: () => qc.invalidateQueries({ queryKey: ['book-issues'] }) });

  function openCreate() { setEditing(null); setForm({ bookId:'', issuedTo:'', issuedDate:'', dueDate:'', returnedDate:'', renewCount:'', fineAmount:'', status:'issued' }); setModalOpen(true); }
  function openEdit(row: any) { setEditing(row); setForm({ bookId: row.bookId?._id||row.bookId||'', issuedTo: row.issuedTo?._id||row.issuedTo||'', issuedDate: row.issuedDate?.slice(0,10)||'', dueDate: row.dueDate?.slice(0,10)||'', returnedDate: row.returnedDate?.slice(0,10)||'', renewCount: String(row.renewCount??''), fineAmount: String(row.fineAmount??''), status: row.status||'issued' }); setModalOpen(true); }
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing?'Edit':'New'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Issued To * <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10}/></Link></label><select required value={form.issuedTo} onChange={e=>setForm(fm=>({...fm,issuedTo:e.target.value}))} className={inp}><option value="">Select...</option>{persons.map((x:any)=><option key={x._id} value={x._id}>{x.name||x._id}</option>)}</select></div>

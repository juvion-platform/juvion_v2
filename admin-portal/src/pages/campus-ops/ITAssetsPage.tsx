...
const emptyForm = { serialNumber:'', type:'desktop', make:'', assetModel:'', ipAddress:'', macAddress:'', location:'', assignedTo:'', purchaseDate:'', warrantyExpiry:'', status:'active' };
...
export default function ITAssetsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['it-assets', page, limit, search], queryFn: () => listITAssets(page, limit, search) });
  const { data: personsData } = useQuery({ queryKey: ['persons-ref','all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ serialNumber: row.serialNumber||'', type: row.type||'desktop', make: row.make||'', assetModel: row.assetModel||'', ipAddress: row.ipAddress||'', macAddress: row.macAddress||'', location: row.location||'', assignedTo: row.assignedTo?._id||row.assignedTo||'', purchaseDate: row.purchaseDate?.slice(0,10)||'', warrantyExpiry: row.warrantyExpiry?.slice(0,10)||'', status: row.status||'active' }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createITAsset, onSuccess: () => { qc.invalidateQueries({ queryKey: ['it-assets'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateITAsset(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['it-assets'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search it assets…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No it assets match “${search}”.` : 'No it assets yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('IT Asset')}>

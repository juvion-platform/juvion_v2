...
const emptyForm = { zone:'', slotNumber:'', type:'two_wheeler', allocatedTo:'', status:'available' };
...
export default function ParkingSlotsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['parking-slots', page, limit, search], queryFn: () => listParkingSlots(page, limit, search) });
  const { data: personsData } = useQuery({ queryKey: ['persons-ref','all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ zone: row.zone||'', slotNumber: row.slotNumber||'', type: row.type||'two_wheeler', allocatedTo: row.allocatedTo?._id||row.allocatedTo||'', status: row.status||'available' }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createParkingSlot, onSuccess: () => { qc.invalidateQueries({ queryKey: ['parking-slots'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateParkingSlot(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['parking-slots'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search parking slots…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No parking slots match “${search}”.` : 'No parking slots yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Parking Slot')}>

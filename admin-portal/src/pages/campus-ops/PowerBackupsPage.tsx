...
export default function PowerBackupsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['power-backups', page, limit, search], queryFn: () => listPowerBackups(page, limit, search) });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search power backups…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No power backups match “${search}”.` : 'No power backups yet.'}
      />

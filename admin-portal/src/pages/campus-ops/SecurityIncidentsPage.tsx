...
const emptyForm = { reportedBy:'', incidentDate:'', location:'', type:'other', description:'', severity:'low', actionTaken:'', status:'reported' };
...
export default function SecurityIncidentsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['security-incidents', page, limit, search], queryFn: () => listSecurityIncidents(page, limit, search) });
  const { data: personsData } = useQuery({ queryKey: ['persons-ref','all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ reportedBy: row.reportedBy?._id||row.reportedBy||'', incidentDate: row.incidentDate?.slice(0,10)||'', location: row.location||'', type: row.type||'other', description: row.description||'', severity: row.severity||'low', actionTaken: row.actionTaken||'', status: row.status||'reported' }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createSecurityIncident, onSuccess: () => { qc.invalidateQueries({ queryKey: ['security-incidents'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateSecurityIncident(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['security-incidents'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search security incidents…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No security incidents match “${search}”.` : 'No security incidents yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Security Incident')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className={lbl}>Reported By * {!vem.isView && <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10}/></Link>}</label><select required value={form.reportedBy} onChange={e=>setForm(fm=>({...fm,reportedBy:e.target.value}))} className={inp}><option value="">Select...</option>{persons.map((x:any)=><option key={x._id} value={x._id}>{x.name||x._id}</option>)}</select></div>

...
const emptyForm = { buildingId:'', roomNumber:'', floor:'', type:'classroom', capacity:'', hasProjector:false, hasAC:false, status:'available' };
...
export default function RoomsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['rooms', page, limit, search], queryFn: () => listRooms(page, limit, search) });
  const { data: bldgData } = useQuery({ queryKey: ['buildings','all'], queryFn: () => listBuildings(1, 100) });
  const buildings = bldgData?.items || [];

  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ buildingId: row.buildingId?._id||row.buildingId||'', roomNumber: row.roomNumber||'', floor: String(row.floor??''), type: row.type||'classroom', capacity: String(row.capacity??''), hasProjector: !!row.hasProjector, hasAC: !!row.hasAC, status: row.status||'available' }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createRoom, onSuccess: () => { qc.invalidateQueries({ queryKey: ['rooms'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateRoom(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['rooms'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search rooms…" className="w-56" />
        <button onClick={vem.openForCreate} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} /> New Room</button>
      </div>
      </div>
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No rooms match “${search}”.` : 'No rooms yet.'}
      />
      <Pagination
        page={page}
        pages={data?.pages ?? 1}
      ...
      />
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Room')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className={lbl}>Building * {!vem.isView && <Link to="/campus-ops/buildings" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10}/></Link>}</label><select required value={form.buildingId} onChange={e=>setForm(f=>({...f,buildingId:e.target.value}))} className={inp}><option value="">Select building</option>{buildings.map((b:any)=><option key={b._id} value={b._id}>{b.name||b.code}</option>)}</select></div>

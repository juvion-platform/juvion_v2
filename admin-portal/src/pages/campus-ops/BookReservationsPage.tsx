...
export default function BookReservationsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['book-reservations', page, limit, search], queryFn: () => listBookReservations(page, limit, search) });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search book reservations…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No book reservations match “${search}”.` : 'No book reservations yet.'}
      />

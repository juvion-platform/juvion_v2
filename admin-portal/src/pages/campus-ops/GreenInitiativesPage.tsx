...
const emptyForm = { name:'', type:'solar', description:'', startDate:'', coordinatorId:'', status:'planned' };
...
export default function GreenInitiativesPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['green-initiatives', page, limit, search], queryFn: () => listGreenInitiatives(page, limit, search) });
  const { data: personsData } = useQuery({ queryKey: ['persons-ref','all'], queryFn: () => listPersons(1, 200) });
  const persons = personsData?.items || [];

  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ name: row.name||'', type: row.type||'solar', description: row.description||'', startDate: row.startDate?.slice(0,10)||'', coordinatorId: row.coordinatorId?._id||row.coordinatorId||'', status: row.status||'planned' }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createGreenInitiative, onSuccess: () => { qc.invalidateQueries({ queryKey: ['green-initiatives'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateGreenInitiative(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['green-initiatives'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search green initiatives…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No green initiatives match “${search}”.` : 'No green initiatives yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Green Initiative')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className={lbl}>Coordinator {!vem.isView && <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10}/></Link>}</label><select value={form.coordinatorId} onChange={e=>setForm(fm=>({...fm,coordinatorId:e.target.value}))} className={inp}><option value="">Select...</option>{persons.map((x:any)=><option key={x._id} value={x._id}>{x.name||x._id}</option>)}</select></div>

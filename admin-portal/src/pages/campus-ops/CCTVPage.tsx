...
const TYPE_OPTS = ['indoor','outdoor','ptz','dome'] as const;
const STATUS_OPTS = ['active','inactive','maintenance'] as const;
...
export default function CCTVPage() {
  ...
  const [form, setForm] = useState({ cameraId:'', location:'', buildingId:'', ipAddress:'', type:'indoor', status:'active', installedDate:'' });

  const { data, isLoading } = useQuery({ queryKey: ['cctvs', page], queryFn: () => listCCTVs(page, 20) });
  const { data: bldgsData } = useQuery({ queryKey: ['buildings-ref','all'], queryFn: () => listBuildings(1, 200) });
  const bldgs = bldgsData?.items || [];

  const createMut = useMutation({ mutationFn: createCCTV, onSuccess: () => { qc.invalidateQueries({ queryKey: ['cctvs'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateCCTV(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['cctvs'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteCCTV, onSuccess: () => qc.invalidateQueries({ queryKey: ['cctvs'] }) });

  function openCreate() { setEditing(null); setForm({ cameraId:'', location:'', buildingId:'', ipAddress:'', type:'indoor', status:'active', installedDate:'' }); setModalOpen(true); }
  function openEdit(row: any) { setEditing(row); setForm({ cameraId: row.cameraId||'', location: row.location||'', buildingId: row.buildingId?._id||row.buildingId||'', ipAddress: row.ipAddress||'', type: row.type||'indoor', status: row.status||'active', installedDate: row.installedDate?.slice(0,10)||'' }); setModalOpen(true); }

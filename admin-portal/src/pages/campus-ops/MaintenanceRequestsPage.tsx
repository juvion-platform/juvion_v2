...
const emptyForm = { requestedBy:'', category:'other', location:'', description:'', priority:'medium', assignedTo:'', status:'open', cost:'' };
...
export default function MaintenanceRequestsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['maintenance-requests', page, limit, search], queryFn: () => listMaintenanceRequests(page, limit, search) });
  ...
  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({ requestedBy: row.requestedBy?._id||row.requestedBy||'', category: row.category||'other', location: row.location||'', description: row.description||'', priority: row.priority||'medium', assignedTo: row.assignedTo?._id||row.assignedTo||'', status: row.status||'open', cost: String(row.cost??'') }),
    onOpenCreate: () => setForm(emptyForm),
    onClose: () => setForm(emptyForm),
  });

  const createMut = useMutation({ mutationFn: createMaintenanceRequest, onSuccess: () => { qc.invalidateQueries({ queryKey: ['maintenance-requests'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateMaintenanceRequest(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['maintenance-requests'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search maintenance requests…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No maintenance requests match “${search}”.` : 'No maintenance requests yet.'}
      />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Maintenance Request')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <fieldset disabled={vem.isView} className="border-0 p-0 m-0 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className={lbl}>Assigned To {!vem.isView && <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10}/></Link>}</label><select value={form.assignedTo} onChange={e=>setForm(fm=>({...fm,assignedTo:e.target.value}))} className={inp}><option value="">Select...</option>{staffList.map((x:any)=><option key={x._id} value={x._id}>{x.person?.name||x.employeeCode||x._id}</option>)}</select></div>

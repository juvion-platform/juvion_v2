...
const CATEGORY_OPTS = ['furniture','electronics','it_equipment','lab_equipment','vehicle','sports','other'] as const;
const STATUS_OPTS = ['in_use','in_stock','maintenance','disposed','lost'] as const;
...
export default function AssetsPage() {
  ...
  const [form, setForm] = useState({ assetId:'', name:'', category:'furniture', departmentId:'', location:'', purchaseDate:'', purchaseCost:'', currentValue:'', vendor:'', warrantyExpiry:'', status:'in_stock' });

  const { data, isLoading } = useQuery({ queryKey: ['assets', page], queryFn: () => listAssets(page, 20) });
  const { data: deptsData } = useQuery({ queryKey: ['depts-ref','all'], queryFn: () => listDepartments(1, 200) });
  const depts = deptsData?.items || [];

  const createMut = useMutation({ mutationFn: createAsset, onSuccess: () => { qc.invalidateQueries({ queryKey: ['assets'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateAsset(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['assets'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteAsset, onSuccess: () => qc.invalidateQueries({ queryKey: ['assets'] }) });

  function openCreate() { setEditing(null); setForm({ assetId:'', name:'', category:'furniture', departmentId:'', location:'', purchaseDate:'', purchaseCost:'', currentValue:'', vendor:'', warrantyExpiry:'', status:'in_stock' }); setModalOpen(true); }
  function openEdit(row: any) { setEditing(row); setForm({ assetId: row.assetId||'', name: row.name||'', category: row.category||'furniture', departmentId: row.departmentId?._id||row.departmentId||'', location: row.location||'', purchaseDate: row.purchaseDate?.slice(0,10)||'', purchaseCost: String(row.purchaseCost??''), currentValue: String(row.currentValue??''), vendor: row.vendor||'', warrantyExpiry: row.warrantyExpiry?.slice(0,10)||'', status: row.status||'in_stock' }); setModalOpen(true); }

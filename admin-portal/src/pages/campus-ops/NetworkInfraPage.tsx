...
export default function NetworkInfraPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['network-infra', page, limit, search], queryFn: () => listNetworkInfra(page, limit, search) });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search network infrastructure…" className="w-56" />
      ...
      <DataTable columns={columns} data={data?.items||[]} loading={isLoading} rowKey={(r: any) => r._id} onRowClick={vem.openForView}
        emptyMessage={search ? `No network infrastructure match “${search}”.` : 'No network infrastructure yet.'}
      />

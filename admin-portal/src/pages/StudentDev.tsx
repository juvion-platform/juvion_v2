...
import { Users, UserPlus, Calendar, Ticket, Award, BookOpen, Trophy, UserCheck, Heart, HeartHandshake, BadgeCheck, FolderGit2, Building2, ShieldCheck, PartyPopper, Swords, Presentation, Flag, Medal, ScrollText, Wallet, Gift } from 'lucide-react';
...
import {
  FestsPage, CompetitionsPage, WorkshopsPage, SDProgrammesPage,
  AwardsPage, CertificatesPage, ActivityBudgetsPage, SponsorshipsPage,
} from './student-dev/w09-pages';
...
function StudentDevHome() {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-navy mb-6">Student Development</h2>
      ...
      {/* Fests, Competitions & Workshops */}
      <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Fests, Competitions & Workshops</h3>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {([
          { to: 'fests', icon: PartyPopper, label: 'Fests', desc: 'Campus fest lifecycle', iconBg: 'bg-fuchsia-50 text-fuchsia-600', border: 'border-fuchsia-200 hover:border-fuchsia-400' },
          { to: 'competitions', icon: Swords, label: 'Competitions', desc: 'Hackathons, quizzes, matches', iconBg: 'bg-orange-50 text-orange-600', border: 'border-orange-200 hover:border-orange-400' },
          { to: 'workshops', icon: Presentation, label: 'Workshops', desc: 'Skill workshops & completion', iconBg: 'bg-sky-50 text-sky-600', border: 'border-sky-200 hover:border-sky-400' },
          { to: 'programmes', icon: Flag, label: 'SD Programmes', desc: 'NCC, NSS, NSO, YRC cohorts', iconBg: 'bg-emerald-50 text-emerald-600', border: 'border-emerald-200 hover:border-emerald-400' },
      ...
      {/* Projects */}
      {/* Recognition & Funding */}
      <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Recognition & Funding</h3>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {([
          { to: 'awards', icon: Medal, label: 'Awards', desc: 'Award catalogue', iconBg: 'bg-amber-50 text-amber-600', border: 'border-amber-200 hover:border-amber-400' },
          { to: 'certificates', icon: ScrollText, label: 'Certificates', desc: 'Issue and revoke certificates', iconBg: 'bg-violet-50 text-violet-600', border: 'border-violet-200 hover:border-violet-400' },
          { to: 'budgets', icon: Wallet, label: 'Activity Budgets', desc: 'Requests, approval, reconciliation', iconBg: 'bg-teal-50 text-teal-600', border: 'border-teal-200 hover:border-teal-400' },
          { to: 'sponsorships', icon: Gift, label: 'Sponsorships', desc: 'Cash and in-kind sponsors', iconBg: 'bg-pink-50 text-pink-600', border: 'border-pink-200 hover:border-pink-400' },
        ] as { to: string; icon: any; label: string; desc: string; iconBg: string; border: string; statKey?: string }[]).map(card => {
          const Icon = card.icon;
          const hasStat = Boolean(card.statKey);
          const count = hasStat && stats ? ((stats as any)[card.statKey!] ?? 0) : null;
  ...
  return (
    <SubPageWrapper>
      <Routes>
        ...
        {/* W09 workflow surfaces — backends shipped without any UI. */}
        <Route path="fests" element={<FestsPage />} />
        <Route path="competitions" element={<CompetitionsPage />} />
        <Route path="workshops" element={<WorkshopsPage />} />
        <Route path="programmes" element={<SDProgrammesPage />} />
        <Route path="awards" element={<AwardsPage />} />
        <Route path="certificates" element={<CertificatesPage />} />
        <Route path="budgets" element={<ActivityBudgetsPage />} />

...
export default function MessagesPage() {
  ...
  const [form, setForm] = useState({ conversationId: '', role: 'user' as string, content: '', intent: '', tokens: '' });
  ...
  const createMut = useMutation({ mutationFn: createMessage, onSuccess: () => { qc.invalidateQueries({ queryKey: ['juvi-messages'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateMessage(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['juvi-messages'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteMessage, onSuccess: () => { qc.invalidateQueries({ queryKey: ['juvi-messages'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ conversationId: '', role: 'user', content: '', intent: '', tokens: '' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Message' : 'New Message'}>

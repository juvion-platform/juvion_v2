...
const emptyForm = { category: '', question: '', answer: '', tags: '', source: '', isActive: true };
...
export default function KnowledgeBasePage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['juvi-knowledge-base', page, limit, search], queryFn: () => listKnowledgeBase(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createKnowledgeBase, onSuccess: () => { qc.invalidateQueries({ queryKey: ['juvi-knowledge-base'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateKnowledgeBase(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['juvi-knowledge-base'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search knowledge base…" className="w-56" />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Knowledge Entry')}>

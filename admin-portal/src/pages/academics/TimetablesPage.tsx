...
export default function TimetablesPage() {
  ...
  const [form, setForm] = useState({ semesterId: '', sectionId: '', version: '1', status: 'draft', effectiveFrom: '' });

  const { data, isLoading } = useQuery({ queryKey: ['timetables', page], queryFn: () => listTimetables(page, 20) });
  ...
  const createMut = useMutation({ mutationFn: createTimetable, onSuccess: () => { qc.invalidateQueries({ queryKey: ['timetables'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateTimetable(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['timetables'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteTimetable, onSuccess: () => qc.invalidateQueries({ queryKey: ['timetables'] }) });

  function openCreate() { setEditing(null); setForm({ semesterId: '', sectionId: '', version: '1', status: 'draft', effectiveFrom: '' }); setModalOpen(true); }
  function openEdit(row: any) {
    setEditing(row);
    setForm({ semesterId: row.semesterId?._id || row.semesterId || '', sectionId: row.sectionId?._id || row.sectionId || '', version: String(row.version || 1), status: row.status || 'draft', effectiveFrom: row.effectiveFrom?.substring(0, 10) || '' });
    setModalOpen(true);
  }
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete timetable and all its slots?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} className="text-white" /> New Timetable</button>
      </div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Timetable' : 'New Timetable'}>

...
export default function AcademicCalendarPage() {
  ...
  const [form, setForm] = useState({ academicYearId: '', title: '', eventType: 'instruction', startDate: '', endDate: '', description: '', isHoliday: false });
  ...
  const createMut = useMutation({ mutationFn: createAcademicCalendar, onSuccess: () => { qc.invalidateQueries({ queryKey: ['academic-calendar'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateAcademicCalendar(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['academic-calendar'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteAcademicCalendar, onSuccess: () => qc.invalidateQueries({ queryKey: ['academic-calendar'] }) });

  function openCreate() { setEditing(null); setForm({ academicYearId: '', title: '', eventType: 'instruction', startDate: '', endDate: '', description: '', isHoliday: false }); setModalOpen(true); }
  function openEdit(row: any) {
    setEditing(row);
    setForm({ academicYearId: row.academicYearId?._id || row.academicYearId || '', title: row.title, eventType: row.eventType, startDate: row.startDate?.substring(0, 10) || '', endDate: row.endDate?.substring(0, 10) || '', description: row.description || '', isHoliday: row.isHoliday || false });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Calendar Event' : 'New Calendar Event'}>

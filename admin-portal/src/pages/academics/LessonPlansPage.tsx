...
export default function LessonPlansPage() {
  ...
  const [form, setForm] = useState({ courseOfferingId: '', weekNumber: '1', topic: '', teachingMethod: '', plannedDate: '', completedDate: '', status: 'planned' });

  const { data, isLoading } = useQuery({ queryKey: ['lesson-plans', page], queryFn: () => listLessonPlans(page, 20) });
  const { data: offeringsData } = useQuery({ queryKey: ['offerings', 1, 200], queryFn: () => listCourseOfferings(1, 200) });

  const createMut = useMutation({ mutationFn: createLessonPlan, onSuccess: () => { qc.invalidateQueries({ queryKey: ['lesson-plans'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateLessonPlan(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['lesson-plans'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteLessonPlan, onSuccess: () => qc.invalidateQueries({ queryKey: ['lesson-plans'] }) });

  function openCreate() { setEditing(null); setForm({ courseOfferingId: '', weekNumber: '1', topic: '', teachingMethod: '', plannedDate: '', completedDate: '', status: 'planned' }); setModalOpen(true); }
  function openEdit(row: any) {
    setEditing(row);
    setForm({ courseOfferingId: row.courseOfferingId?._id || row.courseOfferingId || '', weekNumber: String(row.weekNumber), topic: row.topic, teachingMethod: row.teachingMethod || '', plannedDate: row.plannedDate?.substring(0, 10) || '', completedDate: row.completedDate?.substring(0, 10) || '', status: row.status || 'planned' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} className="text-white" /> New Lesson Plan</button>
      </div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Lesson Plan' : 'New Lesson Plan'}>

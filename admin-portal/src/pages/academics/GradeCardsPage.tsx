...
const emptyForm = { studentId: '', semesterId: '', courseId: '', internalMarks: '0', externalMarks: '0', totalMarks: '0', grade: '', gradePoints: '0', credits: '0', result: 'pass' };
...
export default function GradeCardsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['grade-cards', page, limit, search], queryFn: () => listGradeCards(page, limit, undefined, undefined, search) });
  ...
  const vem = useViewEditMode<any>({
    onOpenEntity: (row) => setForm({
      ...
      externalMarks: String(row.externalMarks),
      totalMarks: String(row.totalMarks),
      grade: row.grade,
      gradePoints: String(row.gradePoints),
  ...
  const createMut = useMutation({ mutationFn: createGradeCard, onSuccess: () => { qc.invalidateQueries({ queryKey: ['grade-cards'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateGradeCard(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['grade-cards'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search grade cards…" className="w-56" />
      ...
      />
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Grade Card')}>

...
export default function FeeLineItemsPage() {
  ...
  const createMut = useMutation({ mutationFn: createFeeLineItem, onSuccess: () => { qc.invalidateQueries({ queryKey: ['fee-line-items'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateFeeLineItem(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['fee-line-items'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteFeeLineItem, onSuccess: () => { qc.invalidateQueries({ queryKey: ['fee-line-items'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ studentId: '', feeStructureId: '', component: '', academicYearId: '', semester: '', amount: '', paidAmount: '', waivedAmount: '', dueDate: '', status: 'pending' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Fee Line Item' : 'New Fee Line Item'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={lbl}>Fee Structure <Link to="/finance/fee-structures" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

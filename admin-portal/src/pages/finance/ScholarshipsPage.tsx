...
const emptyForm = { name: '', provider: 'government', type: 'merit', amount: '', criteria: '', academicYearId: '', maxRecipients: '', isActive: true };
...
export default function ScholarshipsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['scholarships', page, limit, search], queryFn: () => listScholarships(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createScholarship, onSuccess: () => { qc.invalidateQueries({ queryKey: ['scholarships'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateScholarship(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['scholarships'] }); vem.close(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this scholarship?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search scholarships…" className="w-56" />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Scholarship')}>

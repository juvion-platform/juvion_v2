...
export default function ExpensesPage() {
  ...
  const [form, setForm] = useState({ budgetId: '', category: '', description: '', amount: '', vendorName: '', invoiceNumber: '', invoiceDate: '', paidDate: '', status: 'submitted' });
  ...
  const createMut = useMutation({ mutationFn: createExpense, onSuccess: () => { qc.invalidateQueries({ queryKey: ['expenses'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateExpense(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['expenses'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteExpense, onSuccess: () => { qc.invalidateQueries({ queryKey: ['expenses'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ budgetId: '', category: '', description: '', amount: '', vendorName: '', invoiceNumber: '', invoiceDate: '', paidDate: '', status: 'submitted' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Expense' : 'New Expense'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Budget <Link to="/finance/budgets" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

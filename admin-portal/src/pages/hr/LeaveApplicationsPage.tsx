...
export default function LeaveApplicationsPage() {
  ...
  const [form, setForm] = useState({ employeeId: '', leaveTypeId: '', fromDate: '', toDate: '', days: '', reason: '', status: 'applied', remarks: '' });
  ...
  const createMut = useMutation({ mutationFn: createLeaveApplication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['leave-applications'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateLeaveApplication(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['leave-applications'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteLeaveApplication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['leave-applications'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ employeeId: '', leaveTypeId: '', fromDate: '', toDate: '', days: '', reason: '', status: 'applied', remarks: '' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Leave Application' : 'New Leave Application'}>

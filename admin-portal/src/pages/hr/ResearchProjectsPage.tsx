...
export default function ResearchProjectsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['research-projects', page, limit, search], queryFn: () => listResearchProjects(page, limit, undefined, search) });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this research project?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search research projects…" className="w-56" />

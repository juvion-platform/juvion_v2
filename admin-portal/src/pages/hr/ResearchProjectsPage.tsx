...
export default function ResearchProjectsPage() {
  ...
  const createMut = useMutation({ mutationFn: createResearchProject, onSuccess: () => { qc.invalidateQueries({ queryKey: ['research-projects'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateResearchProject(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['research-projects'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteResearchProject, onSuccess: () => { qc.invalidateQueries({ queryKey: ['research-projects'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ title: '', principalInvestigatorId: '', fundingAgency: '', sanctionedAmount: '', startDate: '', endDate: '', status: 'proposed' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Research Project' : 'New Research Project'}>

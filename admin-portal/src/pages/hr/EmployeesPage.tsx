...
export default function EmployeesPage() {
  ...
  const [form, setForm] = useState({ personId: '', employeeId: '', departmentId: '', designation: '', employeeType: 'teaching', joiningDate: '', reportingToId: '', status: 'active' });
  ...
  const createMut = useMutation({ mutationFn: createEmployee, onSuccess: () => { qc.invalidateQueries({ queryKey: ['employees'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateEmployee(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['employees'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteEmployee, onSuccess: () => { qc.invalidateQueries({ queryKey: ['employees'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ personId: '', employeeId: '', departmentId: '', designation: '', employeeType: 'teaching', joiningDate: '', reportingToId: '', status: 'active' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Employee' : 'New Employee'}>

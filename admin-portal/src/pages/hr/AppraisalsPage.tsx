...
export default function AppraisalsPage() {
  ...
  const [form, setForm] = useState({ employeeId: '', academicYearId: '', reviewerId: '', selfRating: '', reviewerRating: '', finalRating: '', status: 'initiated' });
  ...
  const createMut = useMutation({ mutationFn: createAppraisal, onSuccess: () => { qc.invalidateQueries({ queryKey: ['appraisals'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateAppraisal(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['appraisals'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteAppraisal, onSuccess: () => { qc.invalidateQueries({ queryKey: ['appraisals'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ employeeId: '', academicYearId: '', reviewerId: '', selfRating: '', reviewerRating: '', finalRating: '', status: 'initiated' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Appraisal' : 'New Appraisal'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Reviewer * <Link to="/hr/employees" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

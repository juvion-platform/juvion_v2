...
export default function JobApplicationsPage() {
  ...
  const [form, setForm] = useState({ recruitmentId: '', applicantName: '', email: '', phone: '', resumeUrl: '', experience: '', currentDesignation: '', status: 'applied', interviewDate: '', interviewRemarks: '' });
  ...
  const createMut = useMutation({ mutationFn: createJobApplication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['job-applications'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateJobApplication(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['job-applications'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteJobApplication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['job-applications'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ recruitmentId: '', applicantName: '', email: '', phone: '', resumeUrl: '', experience: '', currentDesignation: '', status: 'applied', interviewDate: '', interviewRemarks: '' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Application' : 'New Application'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Recruitment * <Link to="/hr/recruitments" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

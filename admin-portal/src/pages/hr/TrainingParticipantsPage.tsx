...
export default function TrainingParticipantsPage() {
  ...
  const [form, setForm] = useState({ trainingId: '', employeeId: '', status: 'nominated', feedbackRating: '', certificateIssued: false });

  const { data, isLoading } = useQuery({ queryKey: ['training-participants', page], queryFn: () => listTrainingParticipants(page, 20) });
  ...
  const createMut = useMutation({ mutationFn: createTrainingParticipant, onSuccess: () => { qc.invalidateQueries({ queryKey: ['training-participants'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateTrainingParticipant(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['training-participants'] }); closeModal(); } });
  ...
  function openCreate() {
    setEditing(null);
    setForm({ trainingId: '', employeeId: '', status: 'nominated', feedbackRating: '', certificateIssued: false });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Participant' : 'New Participant'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className={lbl}>Training * <Link to="/hr/trainings" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

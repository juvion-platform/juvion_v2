...
export default function GrievancesPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['grievances', page, limit, search], queryFn: () => listGrievances(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createGrievance, onSuccess: () => { qc.invalidateQueries({ queryKey: ['grievances'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateGrievance(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['grievances'] }); vem.close(); } });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search grievances…" className="w-56" />

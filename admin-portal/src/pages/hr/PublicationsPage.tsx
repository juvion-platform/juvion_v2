...
export default function PublicationsPage() {
  ...
  const createMut = useMutation({ mutationFn: createPublication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['publications'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updatePublication(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['publications'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deletePublication, onSuccess: () => { qc.invalidateQueries({ queryKey: ['publications'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ facultyId: '', title: '', type: 'journal', journalName: '', conferenceName: '', publishedDate: '', doi: '', impactFactor: '', indexing: '' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Publication' : 'New Publication'}>

...
export default function ExitProcessPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['exit-processes', page, limit, search], queryFn: () => listExitProcesses(page, limit, undefined, search) });
  ...
  const createMut = useMutation({ mutationFn: createExitProcess, onSuccess: () => { qc.invalidateQueries({ queryKey: ['exit-processes'] }); vem.close(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateExitProcess(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['exit-processes'] }); vem.close(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); void confirmAction({ title: 'Delete this exit process?', tone: 'danger', confirmLabel: 'Delete' }).then((__c) => { if (__c.confirmed) { deleteMut.mutate(r._id); } }) }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <SearchInput value={search} onChange={setSearch} placeholder="Search exit processes…" className="w-56" />
      ...
      <Modal open={vem.isOpen} onClose={vem.close} title={vem.titleFor('Exit Process')}>

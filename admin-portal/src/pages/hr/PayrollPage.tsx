...
export default function PayrollPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['payroll', page], queryFn: () => listPayrolls(page, 20) });
  const { data: empData } = useQuery({ queryKey: ['employees-all'], queryFn: () => listEmployees(1, 200) });

  const employees = empData?.items || [];

  const createMut = useMutation({ mutationFn: createPayroll, onSuccess: () => { qc.invalidateQueries({ queryKey: ['payroll'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updatePayroll(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['payroll'] }); closeModal(); } });
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this payroll record?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Payroll' : 'New Payroll'}>

...
const STAFF_TYPES = ['Administrative', 'Technical', 'Lab Assistant', 'Library', 'Accounts', 'Maintenance', 'Security', 'Other'] as const;
...
export default function StaffFormPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['staff'] }); qc.invalidateQueries({ queryKey: ['people-stats'] }); navigate('/people/staff'); },
  });
  const updateMut = useMutation({
    mutationFn: ({ id, data }: any) => updateStaff(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['staff'] }); qc.invalidateQueries({ queryKey: ['staff-single', id] }); navigate('/people/staff'); },
  });
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/people/staff')} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500"><ArrowLeft size={20} className="text-gray-500" /></button>
          <div>
            <h2 className="text-xl font-bold text-navy">{isEdit ? 'Edit Staff' : 'New Staff'}</h2>
            {isEdit && existing && (
      ...
      <form id="staff-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white rounded-xl border shadow-sm">
          <div className="px-5 py-4 border-b bg-navy/[0.03] rounded-t-xl">
            ...
            <p className="text-xs text-gray-500 mt-0.5">Role and department information</p>
          </div>
          <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-4">

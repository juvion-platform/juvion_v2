...
export default function SemestersPage() {
  ...
  const [form, setForm] = useState({ academicYearId: '', number: '', year: '', startDate: '', endDate: '', status: 'upcoming' });
  ...
  const createMut = useMutation({ mutationFn: createSemester, onSuccess: () => { qc.invalidateQueries({ queryKey: ['semesters'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateSemester(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['semesters'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteSemester, onSuccess: () => { qc.invalidateQueries({ queryKey: ['semesters'] }); } });

  function openCreate() { setEditing(null); setForm({ academicYearId: '', number: '', year: '', startDate: '', endDate: '', status: 'upcoming' }); setModalOpen(true); }
  function openEdit(row: any) {
    setEditing(row);
    setForm({ academicYearId: row.academicYearId?._id || row.academicYearId || '', number: String(row.number), year: String(row.year), startDate: row.startDate?.substring(0, 10) || '', endDate: row.endDate?.substring(0, 10) || '', status: row.status });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Semester' : 'New Semester'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Semester Number *</label><input required type="number" min={1} max={12} value={form.number} onChange={e => setForm(f => ({ ...f, number: e.target.value }))} className={inp} /></div>
            <div><label className={lbl}>Year *</label><input required type="number" min={1} max={6} value={form.year} onChange={e => setForm(f => ({ ...f, year: e.target.value }))} className={inp} /></div>

...
export default function RegulationsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['regulations', page], queryFn: () => listRegulations(page, 20) });

  const createMut = useMutation({ mutationFn: createRegulation, onSuccess: () => { qc.invalidateQueries({ queryKey: ['regulations'] }); closeModal(); } });
  ...
  const deleteMut = useMutation({ mutationFn: deleteRegulation, onSuccess: () => { qc.invalidateQueries({ queryKey: ['regulations'] }); } });

  function openCreate() { setEditing(null); setForm({ code: '', name: '', effectiveFromYear: '', effectiveToYear: '', totalCredits: '', maxYears: '', isActive: true }); setModalOpen(true); }
  ...
  const columns = [
    ...
    { key: 'name', label: 'Name' },
    { key: 'effectiveFromYear', label: 'From' },
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 flex items-center gap-2">
              <input type="checkbox" id="regIsActive" checked={form.isActive} onChange={e => setForm(f => ({ ...f, isActive: e.target.checked }))} className="rounded" />
              <label htmlFor="regIsActive" className="text-sm text-gray-700">Active</label>
            </div>
          </div>

...
export default function CoursesPage() {
  ...
  const createMut = useMutation({ mutationFn: createCourse, onSuccess: () => { qc.invalidateQueries({ queryKey: ['courses'] }); closeModal(); } });
  ...
  const deleteMut = useMutation({ mutationFn: deleteCourse, onSuccess: () => { qc.invalidateQueries({ queryKey: ['courses'] }); } });

  function openCreate() { setEditing(null); setForm({ code: '', name: '', regulationId: '', departmentId: '', credits: '', lectureHrs: '3', tutorialHrs: '0', practicalHrs: '0', type: 'theory', isElective: false }); setModalOpen(true); }

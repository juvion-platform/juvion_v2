...
export default function EventRegistrationsPage() {
  ...
  const [form, setForm] = useState({ eventId: '', participantId: '', participantType: 'student', teamName: '', status: 'registered' });

  const { data, isLoading } = useQuery({ queryKey: ['sd-event-registrations', page], queryFn: () => listEventRegistrations(page, 20) });
  ...
  const createMut = useMutation({ mutationFn: createEventRegistration, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-event-registrations'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateEventRegistration(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-event-registrations'] }); closeModal(); } });
  ...
  function openCreate() {
    setEditing(null);
    setForm({ eventId: '', participantId: '', participantType: 'student', teamName: '', status: 'registered' });
    setModalOpen(true);
  }
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Registration' : 'New Registration'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Event * <Link to="/student-dev/events" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>
            ...
            <div>
              <label className={lbl}>Participant * <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

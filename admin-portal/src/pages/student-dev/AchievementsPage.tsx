...
export default function AchievementsPage() {
  ...
  const [form, setForm] = useState({ studentId: '', title: '', category: 'academic', level: 'college', date: '', description: '', certificateUrl: '', verifiedBy: '' });

  const { data, isLoading } = useQuery({ queryKey: ['sd-achievements', page], queryFn: () => listAchievements(page, 20) });
  ...
  const createMut = useMutation({ mutationFn: createAchievement, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-achievements'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateAchievement(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-achievements'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteAchievement, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-achievements'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ studentId: '', title: '', category: 'academic', level: 'college', date: '', description: '', certificateUrl: '', verifiedBy: '' });
    setModalOpen(true);
  }
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this achievement?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Achievement' : 'New Achievement'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={lbl}>Verified By <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

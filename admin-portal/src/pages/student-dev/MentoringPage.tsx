...
export default function MentoringPage() {
  ...
  const [form, setForm] = useState({ mentorId: '', menteeId: '', academicYearId: '', meetingDate: '', notes: '', status: 'active' });

  const { data, isLoading } = useQuery({ queryKey: ['sd-mentoring', page], queryFn: () => listMentoringSessions(page, 20) });
  ...
  const createMut = useMutation({ mutationFn: createMentoringSession, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-mentoring'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateMentoringSession(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-mentoring'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteMentoringSession, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-mentoring'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ mentorId: '', menteeId: '', academicYearId: '', meetingDate: '', notes: '', status: 'active' });
    setModalOpen(true);
  }
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this mentoring session?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Mentoring Session' : 'New Mentoring Session'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Mentor (Faculty) * <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>
            ...
            <div>
              <label className={lbl}>Mentee (Student) * <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

...
export default function EventsPage() {
  ...
  const [form, setForm] = useState({ name: '', type: 'technical', clubId: '', departmentId: '', description: '', startDate: '', endDate: '', venue: '', budget: '', coordinatorId: '', status: 'planned' });
  ...
  const createMut = useMutation({ mutationFn: createEvent, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-events'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateEvent(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-events'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteEvent, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-events'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ name: '', type: 'technical', clubId: '', departmentId: '', description: '', startDate: '', endDate: '', venue: '', budget: '', coordinatorId: '', status: 'planned' });
    setModalOpen(true);
  }
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Event' : 'New Event'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={lbl}>Department <Link to="/academics" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>
            ...
            <div>
              <label className={lbl}>Coordinator <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

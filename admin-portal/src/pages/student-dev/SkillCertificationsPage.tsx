...
export default function SkillCertificationsPage() {
  ...
  const [form, setForm] = useState({ studentId: '', certificationName: '', provider: '', completedDate: '', certificateUrl: '', credentialId: '', validUntil: '' });

  const { data, isLoading } = useQuery({ queryKey: ['sd-skill-certifications', page], queryFn: () => listSkillCertifications(page, 20) });
  const { data: students } = useQuery({ queryKey: ['students', 'all'], queryFn: () => listStudents(1, 200) });

  const createMut = useMutation({ mutationFn: createSkillCertification, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-skill-certifications'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateSkillCertification(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-skill-certifications'] }); closeModal(); } });
  ...
  function openCreate() {
    setEditing(null);
    setForm({ studentId: '', certificationName: '', provider: '', completedDate: '', certificateUrl: '', credentialId: '', validUntil: '' });
    setModalOpen(true);
  }
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this certification?')) deleteMut.mutate(r._id); }} className="p-1 rounded hover:bg-red-50" title="Delete"><Trash2 size={15} className="text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Certification' : 'New Certification'}>

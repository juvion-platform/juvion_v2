...
export default function StudentProjectsPage() {
  ...
  const [form, setForm] = useState({ title: '', type: 'mini_project', teamMembers: '', guideId: '', semester: '', description: '', technologies: '', repoUrl: '', status: 'proposed', grade: '' });

  const { data, isLoading } = useQuery({ queryKey: ['sd-student-projects', page], queryFn: () => listStudentProjects(page, 20) });
  const { data: faculty } = useQuery({ queryKey: ['faculty', 'all'], queryFn: () => listFaculty(1, 200) });

  const createMut = useMutation({ mutationFn: createStudentProject, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-student-projects'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateStudentProject(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-student-projects'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteStudentProject, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-student-projects'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ title: '', type: 'mini_project', teamMembers: '', guideId: '', semester: '', description: '', technologies: '', repoUrl: '', status: 'proposed', grade: '' });
    setModalOpen(true);
  }
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Project' : 'New Project'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={lbl}>Guide (Faculty) <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>

...
export default function NSSActivitiesPage() {
  ...
  const [form, setForm] = useState({ title: '', type: 'camp', date: '', venue: '', description: '', coordinatorId: '', participantCount: '', hours: '', status: 'planned' });
  ...
  const createMut = useMutation({ mutationFn: createNSSActivity, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-nss-activities'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateNSSActivity(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-nss-activities'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteNSSActivity, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-nss-activities'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ title: '', type: 'camp', date: '', venue: '', description: '', coordinatorId: '', participantCount: '', hours: '', status: 'planned' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit NSS Activity' : 'New NSS Activity'}>

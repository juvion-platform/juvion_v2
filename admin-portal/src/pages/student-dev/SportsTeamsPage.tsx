...
export default function SportsTeamsPage() {
  ...
  const [form, setForm] = useState({ sport: '', category: 'men', coachId: '', captain: '', academicYearId: '' });
  ...
  const createMut = useMutation({ mutationFn: createSportsTeam, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-sports-teams'] }); closeModal(); } });
  const updateMut = useMutation({ mutationFn: ({ id, data }: any) => updateSportsTeam(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-sports-teams'] }); closeModal(); } });
  const deleteMut = useMutation({ mutationFn: deleteSportsTeam, onSuccess: () => { qc.invalidateQueries({ queryKey: ['sd-sports-teams'] }); } });

  function openCreate() {
    setEditing(null);
    setForm({ sport: '', category: 'men', coachId: '', captain: '', academicYearId: '' });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Sports Team' : 'New Sports Team'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={lbl}>Coach <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>
            ...
            <div>
              <label className={lbl}>Captain <Link to="/people" target="_blank" className={manageLink}>+ Manage <ExternalLink size={10} /></Link></label>
